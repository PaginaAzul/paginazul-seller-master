...
export class CommissionManagementComponent implements OnInit {
  ...
  getViewData() {
    ...
    this.spinner.show();
    this.service.postApi('/api/v1/user/getCommissionListForSeller', apireq, 0).subscribe((success: any) => {
  ...
  search(data) {
    ...
    this.service.postApi('/api/v1/user/getCommissionListForSeller', apireq, 1).subscribe((success: any) => {
      if (success.response_code == 200) {
        this.spinner.hide();
        this.recordArray = success.Data.docs;
        this.limit = success.Data.limit;
