...
@Component({
  ...
  styleUrls: ['./cuisine-management.component.scss']
})
export class CuisineManagementComponent implements OnInit {
  ...
  getViewData() {
    ...
    this.spinner.show();
    this.service.postApi('/api/v1/user/getCuisineListForSeller', apireq, 0).subscribe((success: any) => {
  ...
  addCuisine() {
    ...
    this.service.formdataApi("/api/v1/user/addCuisine", formData).subscribe((data: any) => {
  ...
  editCuisine() {
    ...
    formData.append("cuisineId", this.cuisineId)
    if (this.profileUrl.length > 0) {
      formData.append(`image`, this.profileUrl[0])
    }

    this.service.formdataApi("/api/v1/user/updateCuisine", formData).subscribe((data: any) => {
      if (data.body.response_code == 200) {
        this.getViewData()
        $("#toggles").modal('hide')
        this.service.succ(data.body.response_message)
  ...
  blockApi(status, id) {
    ...
    this.spinner.show();
    this.service.postApi('/api/v1/user/updateStatusCuisine', apireq, 0).subscribe((success: any) => {
      if (success.response_code == 200) {
        this.spinner.hide()
        this.getViewData()
